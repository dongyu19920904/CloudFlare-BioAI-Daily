import { formatMarkdownText, formatDateToGMT8WithTime, removeMarkdownCodeBlock, sanitizeMarkdownImages } from '../helpers.js';
...
export async function handleWriteRssData(request, env) {
    ...
    try {
        ...
        const normalizedContent = await sanitizeMarkdownImages(formatMarkdownText(content, env.IMG_PROXY));
        report.content_html = marked.parse(normalizedContent);
